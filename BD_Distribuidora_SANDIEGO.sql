...
/*Table structure for table `t_kardex` */

DROP TABLE IF EXISTS `t_kardex`;

CREATE TABLE `t_kardex` (
  `kar_id` varchar(20) NOT NULL COMMENT 'ID del registro kardex',
  `kar_pro_id_fk` varchar(20) DEFAULT NULL COMMENT 'ID del producto',
  `kar_lot_id_fk` varchar(20) DEFAULT NULL COMMENT 'ID del lote',
  `kar_inm_id_fk` varchar(20) DEFAULT NULL COMMENT 'ID del movimiento que generó este registro',
  `kar_fecha` date DEFAULT NULL COMMENT 'Fecha del movimiento',
  `kar_tipo` varchar(20) DEFAULT NULL COMMENT 'Entrada / Salida / Ajuste',
  `kar_cantidad` int(11) DEFAULT NULL COMMENT 'Cantidad del movimiento',
  `kar_saldo_anterior` int(11) DEFAULT NULL COMMENT 'Stock antes del movimiento',
  `kar_saldo_actual` int(11) DEFAULT NULL COMMENT 'Stock después del movimiento',
  `kar_costo_unitario` decimal(12,2) DEFAULT NULL COMMENT 'Costo unitario del producto',
  `kar_costo_total` decimal(12,2) DEFAULT NULL COMMENT 'Costo total del movimiento',
  PRIMARY KEY (`kar_id`),
  KEY `kar_pro_id_fk` (`kar_pro_id_fk`),
  KEY `kar_lot_id_fk` (`kar_lot_id_fk`),
  KEY `kar_inm_id_fk` (`kar_inm_id_fk`),
  CONSTRAINT `kardex_ibfk_1` FOREIGN KEY (`kar_pro_id_fk`) REFERENCES `t_producto` (`pro_id`),
  CONSTRAINT `kardex_ibfk_2` FOREIGN KEY (`kar_lot_id_fk`) REFERENCES `t_lote` (`lot_id`),
  CONSTRAINT `kardex_ibfk_3` FOREIGN KEY (`kar_inm_id_fk`) REFERENCES `t_inventario_movimiento` (`inm_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

/*Data for the table `t_kardex` */

insert  into `t_kardex`(`kar_id`,`kar_pro_id_fk`,`kar_lot_id_fk`,`kar_inm_id_fk`,`kar_fecha`,`kar_tipo`,`kar_cantidad`,`kar_saldo_anterior`,`kar_saldo_actual`,`kar_costo_unitario`,`kar_costo_total`) values 
('KAR001','PRO001','LOT001','INM001','2025-03-10','Entrada',100,100,200,850.00,85000.00),
('KAR002','PRO001','LOT001','INM002','2025-03-15','Salida',5,200,195,850.00,4250.00),
('KAR003','PRO002','LOT002','INM003','2025-03-16','Salida',3,150,147,1200.00,3600.00),
('KAR004','PRO003','LOT003','INM004','2025-03-12','Entrada',50,30,80,980.00,49000.00),
('KAR005','PRO005','LOT005','INM005','2025-03-18','Salida',1,45,44,4200.00,4200.00),
('KAR050','PRO050','LOT050','INM050','2026-04-09','Entrada',100,200,300,8500.00,850000.00),
('KAR051','PRO051','LOT051','INM051','2026-04-09','Entrada',150,150,300,15000.00,2250000.00),
('KAR052','PRO052','LOT052','INM052','2026-04-09','Salida',10,300,290,5200.00,52000.00);
...
CREATE TABLE `t_usuario` (
  ...
  `usu_rol` varchar(50) DEFAULT NULL COMMENT 'Rol: Administrador / Vendedor / Bodeguero / Contador',
...
insert  into `t_usuario`(`usu_id`,`usu_nombre`,`usu_rol`,`usu_correo`,`usu_contrasena`,`usu_estado`,`usu_ultimo_acceso`) values 
('USU001','Diana Lopez','Administrador','d.lopez@sd.com','Admin#2025',1,NULL),
('USU002','Pedro Vargas','Vendedor','p.vargas@sd.com','Vend#Pedro1',1,'2025-03-19 08:05:00'),
('USU003','Sandra Nino','Vendedor','s.nino@sd.com','Vend#Sandra2',1,'2025-03-19 08:10:00'),
('USU004','Camilo Ruiz','Bodeguero','c.ruiz@sd.com','Bodeg#Cam3',1,'2025-03-12 07:55:00'),
('USU005','Luisa Mora','Contador','l.mora@sd.com','Cont#Luisa4',1,NULL),
('USU050','Andres Gomez','Vendedor','andres.gomez@farmacia.com','Andres#2026',1,NULL),
('USU051','Maria Torres','Administrador','maria.torres@farmacia.com','Maria#2026',1,NULL),
('USU052','Pedro Ruiz','Bodeguero','pedro.ruiz@farmacia.com','Pedro#2026',1,NULL),
('USU099','Test User','Vendedor','test@sd.com','Test#2026',1,NULL);
